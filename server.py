...
async def execute_word_call(room_id, word_text, caller_nickname):
        ...
        if all_marked:
            if game_mode == 'WINNER':
                if not game_over:
                    ...
                    sys_msg_win = f"🏆 '{p['nickname']}'님이 모든 칸을 완성하여 1등 승리자가 되었습니다! 🎉"
    ...
    if game_mode == 'LOSER' and not game_over:
        ...
        if total_players > 1 and len(un_escaped) == 1:
            loser_player = un_escaped[0]
            loser_player['is_loser'] = True
            game_over = True
            room['status'] = 'GAME_OVER'
            sys_msg_lose = f"💣 '{loser_player['nickname']}'님이 마지막까지 남아 최종 패자(벌칙 당첨자)가 되었습니다! 😭"
            room['chat_logs'].append({'system': True, 'text': sys_msg_lose})
            game_result = {
                'mode': 'LOSER',
                'loser_id': loser_player['id'],
                'loser_nickname': loser_player['nickname'],
                'loser_color': loser_player['color']
            }
